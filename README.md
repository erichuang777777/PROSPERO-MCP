...
This installs the `prospero`, `prospero-mcp`, `prospero-mcp-setup`, `prospero-mcp-login` and `prospero-mcp-install-skill` executables globally. Pin a specific release with `github:erichuang777777/PROSPERO-MCP#v0.2.2`.
...
The one-command install above is the shared base for every agent (it also provides `prospero-mcp-setup` and `prospero-mcp-login`). Then wire it into your agent:

### Codex

Install the skill (also installs the Claude Code copy; use `--codex` to restrict to Codex):

```powershell
prospero-mcp-install-skill
```

Add the MCP server to `~/.codex/config.toml` — use the entry generated by `prospero-mcp-setup` (`.prospero-mcp.generated.json`), which pins the exact `node` + server path and safe env defaults. Restart Codex, then call `prospero_health`.

### Claude Code

Install the bundled plugin (MCP server + skill in one step):
...
The plugin's MCP server runs the globally installed `prospero-mcp` bin, so keep the one-command install above in place. Prefer manual MCP wiring instead? Copy the `prospero-mcp-setup`-generated entry into your MCP settings.

The skill installer and plugin both refuse to overwrite an existing skill unless `--force` is supplied; restart the agent after installing.
